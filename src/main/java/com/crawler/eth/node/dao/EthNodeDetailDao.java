...
public interface EthNodeDetailDao extends JpaRepository<EthNodeDetailModel,Long>{
    ...
    @Query(value="select u from EthNodeDetailModel u where u.nodeType = ?1 and u.enabled = ?2")
    List<EthNodeDetailModel> findByNodeTypeAndEnabled(String nodeType, Integer enabled);
